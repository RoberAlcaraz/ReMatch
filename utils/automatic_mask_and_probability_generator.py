...
class SamAutomaticMaskAndProbabilityGenerator(SamAutomaticMaskGenerator):
    ...
    def _process_batch(
      ...
      try:
        return self._process_batch_impl(points, im_size, crop_box, orig_size)
        ...
        logging.warning(
          "CUDA OOM in SAM batch with %d points; retrying with %d + %d.",
          n_points,
          split_idx,
          n_points - split_idx,
        )
        left_data = self._process_batch(points[:split_idx], im_size, crop_box, orig_size)
        right_data = self._process_batch(points[split_idx:], im_size, crop_box, orig_size)
        left_data.cat(right_data)
        del right_data
        return left_data
