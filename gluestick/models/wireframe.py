...
class SPWireframeDescriptor(BaseModel):
    ...
    def _forward(self, data, save_path=None, image_id=None):
        # Check if precomputed data is available
        if save_path and image_id:
        ...
        # Save the computed lines and wireframe if `hdf5_path` is provided
        if save_path and image_id:
            with h5py.File(save_path, "a") as hdf5_file:
                grp = hdf5_file.require_group(image_id)
                grp.create_dataset("image_size", data=(1, 1, h, w), compression="gzip")
                grp.create_dataset(
                    "keypoints",
                    data=all_points.cpu().detach().numpy(),
                    compression="gzip",
                )
                grp.create_dataset(
                    "keypoint_scores",
                    data=all_scores.cpu().detach().numpy(),
                    compression="gzip",
                )
                grp.create_dataset(
                    "descriptors",
                    data=all_descs.cpu().detach().numpy(),
                    compression="gzip",
                )
                grp.create_dataset(
                    "lines", data=lines.cpu().detach().numpy(), compression="gzip"
                )
                grp.create_dataset(
                    "line_scores",
                    data=line_scores.cpu().detach().numpy(),
                    compression="gzip",
                )
                grp.create_dataset(
                    "pl_associativity",
                    data=pl_associativity.cpu().detach().numpy(),
                    compression="gzip",
                )
                grp.create_dataset(
                    "lines_junc_idx",
                    data=lines_junc_idx.cpu().detach().numpy(),
                    compression="gzip",
                )

        return {
            "image_size": torch.Size([1, 1, h, w]),
