...
class SuperPoint(BaseModel):
    ...
    def _forward(self, data, save_path=None, image_id=None):
        # If `hdf5_path` is provided, attempt to load precomputed data
        ...
        # Save the computed data to HDF5 if `hdf5_path` is provided
        if save_path and image_id:
            with h5py.File(save_path, "a") as hdf5_file:
                grp = hdf5_file.require_group(image_id)
                grp.create_dataset(
                    "keypoints",
                    data=keypoints.cpu().detach().numpy(),
                    compression="gzip",
                )
                grp.create_dataset(
                    "keypoint_scores",
                    data=scores.cpu().detach().numpy(),
                    compression="gzip",
                )
                grp.create_dataset(
                    "descriptors", data=desc.cpu().detach().numpy(), compression="gzip"
                )
